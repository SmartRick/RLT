  ...
  try {
    ...
    const formData = {
      ...
      lora_training: assetForm.value.lora_training.enabled 
        ...
        : { enabled: false, url: 'none', port: null, config_path: '', params: {} },
      ai_engine: assetForm.value.ai_engine.enabled
        ? assetForm.value.ai_engine
        : { enabled: false, url: 'none', port: null }
    }

    // 确保 params 是对象类型
    if (typeof formData.lora_training.params === 'string') {
      try {
        formData.lora_training.params = JSON.parse(formData.lora_training.params)
      } catch (e) {
        formData.lora_training.params = {}
      }
    }
