...
const handleSubmit = async () => {
  ...
  try {
    ...
    const formData = {
      ...
      // 根据认证类型设置认证信息
      ...(assetForm.value.ssh_auth_type === 'PASSWORD' 
    ...
    if (isEditing.value) {
      await assetApi.updateAsset(assetForm.value.id, formData)
...
const showEditModal = (asset) => {
  ...
  // 确保认证类型字段存在
  assetData.ssh_auth_type = assetData.ssh_auth_type || 'KEY'
...
const formRules = {
  ...
  ssh_password: [
    { 
      required: true, 
      message: '请输入SSH密码',
      when: (form) => form.ssh_auth_type === 'PASSWORD'
    }
  ],
  ssh_key_path: [
    { 
      required: true, 
      message: '请输入SSH密钥路径',
      when: (form) => form.ssh_auth_type === 'KEY'
    }
  ]
}
