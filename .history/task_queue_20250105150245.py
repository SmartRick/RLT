...
class TaskStatus:
    ...
    TRAINING_COMPLETED = "training_completed"  # 训练完成，等待上传
    UPLOADING = "uploading"             # 上传中
    UPLOAD_FAILED = "upload_failed"      # 上传失败
    COMPLETED = "completed"              # 全部完成（训练和上传都完成）
    PENDING_UPLOAD = "pending_upload"  # 等待上传
...
class TaskQueue:
    ...
    def mark_training_complete(self, folder_name, task_info):
        """
        @description 标记训练完成（但还未上传）
        @param {str} folder_name - 文件夹名称
        @param {dict} task_info - 任务信息，包含lora_path和task_id
        """
        task_info["status"] = TaskStatus.TRAINING_COMPLETED
        task_info["updated_at"] = datetime.now().isoformat()
        self.update_task_status(folder_name, TaskStatus.TRAINING_COMPLETED, **task_info)
    ...
    def mark_uploading(self, folder_name):
        """
        @description 标记任务为上传中
        @param {str} folder_name - 文件夹名称
        """
        self.update_task_status(folder_name, TaskStatus.UPLOADING)
        
    def mark_upload_failed(self, folder_name, error_msg):
        """
        @description 标记任务上传失败
        @param {str} folder_name - 文件夹名称
        @param {str} error_msg - 错误信息
        """
        self.update_task_status(folder_name, TaskStatus.UPLOAD_FAILED, {
            "error": error_msg,
            "failed_at": datetime.now().isoformat()
        })
        
    def is_upload_failed(self, folder_name):
        """
        @description 检查任务是否为上传失败状态
        @param {str} folder_name - 文件夹名称
        @return {bool} 是否为上传失败状态
        """
        task = self.get_task(folder_name)
        return task and task.get("status") == TaskStatus.UPLOAD_FAILED
        
    def mark_complete(self, folder_name):
        """
        @description 标记任务完全完成（训练和上传都成功）
        @param {str} folder_name - 文件夹名称
        """
        self.update_task_status(folder_name, TaskStatus.COMPLETED)

    def get_training_completed_tasks(self):
        """
        @description 获取训练已完成但未上传的任务
        @return {list} 任务列表
        """
        return self.get_tasks_by_status(TaskStatus.TRAINING_COMPLETED) 

    def add_to_upload_queue(self, folder_name):
        """将任务添加到上传队列"""
        self.update_task_status(folder_name, TaskStatus.PENDING_UPLOAD)
