...
class Asset(AssetCreate):
    ...
    def dict(self, *args, **kwargs):
        # 自定义dict方法，确保返回前端需要的格式
        base_dict = super().dict(*args, **kwargs)
        
        # 简化 lora_training 和 ai_engine 的输出
        if 'lora_training' in base_dict:
            base_dict['lora_training'] = {
                'enabled': base_dict['lora_training']['enabled'],
                'verified': base_dict['lora_training'].get('verified', False)
            }
            
        if 'ai_engine' in base_dict:
            base_dict['ai_engine'] = {
                'enabled': base_dict['ai_engine']['enabled'],
                'verified': base_dict['ai_engine'].get('verified', False)
            }
