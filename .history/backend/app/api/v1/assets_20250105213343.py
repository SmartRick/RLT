...
def list_assets():
    ...
    return jsonify([asset.dict() for asset in assets])
...
def update_asset(asset_id):
    """更新资产"""
    try:
        logger.debug(f"更新资产: {asset_id}, 数据: {request.json}")
        update_data = AssetUpdate(**request.json)
        asset = AssetService.update_asset(asset_id, update_data)
        if asset:
            return jsonify(asset.dict())
        logger.error(f"资产不存在或更新失败: {asset_id}")
        return jsonify({'error': '资产不存在或更新失败'}), 404
    except ValueError as e:
        logger.error(f"更新资产验证错误: {str(e)}")
        return jsonify({'error': str(e)}), 400
    except Exception as e:
        logger.error(f"更新资产异常: {str(e)}")
...
def verify_ssh_connection():
    ...
    try:
        data = request.json
        result = AssetService.verify_ssh_connection(data)
