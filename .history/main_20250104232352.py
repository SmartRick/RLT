...
def submit_task(scan_dir, request_json_path):
    ...
    print(f"当前已完成任务数: {len(finished_folder)}")
    print(f"当前已下载素材数: {len(mark_downloads)}")
    
    for folder in os.listdir(scan_dir):
        if folder in finished_folder:
            print(f"跳过已完成的任务: {folder}")
            continue
        if folder not in mark_downloads:
            print(f"跳过未下载完成的素材: {folder}")
            continue
...
def scheduling(scan_dir, request_json_path):
    ...
    if task_map:
        ...
        for taskid, state in task_map.items():
            print(f"任务ID: {taskid} - 状态: {state}")
            if state == "RUNNING":
                print(f"发现正在执行的任务，等待完成...")
                return

    if not cur_task_id:
        print("没有进行中的任务，准备提交新任务...")
