...
class LoraTrainingSystem:
        ...
        while True:
            ...
            try:
                ...
                if success == 0:
                    ...
                    raise Exception(f"下载失败 - 错误码: {success}")
                
            except Exception as e:
                logging.error(f"素材下载失败 - {folder_name} - 错误: {str(e)}")
                self.task_queue.mark_download_failed(folder_name, str(e))
    ...
    def _scan_and_submit_tasks(self):
        ...
        for folder in os.listdir(self.scan_dir):
            ...
            if os.path.isdir(folder_path):
                ...
                if self.task_queue.add_task(task_info):
                    tasks_added = True
        
        # 如果添加了新任务或队列不为空，尝试提交下一个任务
        if tasks_added or self.task_queue.get_queue_length() > 0:
    ...
    def _submit_next_task(self):
        ...
        try:
            ...
            # 注册任务并更新状态
            self._register_task(task_id, folder)
            self.task_queue.update_task_status(folder, "running")
